                ...
                if custom_update_statements:
                    for update_spec in custom_update_statements:
                        ...
                        if row_idx < len(all_rows):
                            ...
                            count_statement, update_statement = generate_update_statement(
                            ...
                            if count_statement and update_statement:
                                yaml_data['sql'].append(count_statement)
...
def generate_update_statement(row, update_col, update_val, columns, key_cols, common_cols, dest_schema, dest_table, dest_connection_name, dest_service_name, record_description=""):
    """Generate UPDATE statement and corresponding count statement for a given row and column"""
    # Get column metadata
    ...
    if where_conditions:
        ...
        count_where_conditions = where_conditions.copy()
        if current_value is not None:
            if isinstance(current_value, str):
                escaped_current = current_value.replace("'", "''")
                count_where_conditions.append(f"{update_col} = '{escaped_current}'")
            else:
                count_where_conditions.append(f"{update_col} = {current_value}")
        else:
            count_where_conditions.append(f"{update_col} IS NULL")
        
        count_where_clause = ' AND '.join(count_where_conditions)
        
        count_statement = {
            'description': f'Count records to be updated for {update_col} = {set_value} in {record_description} of {dest_schema}.{dest_table}',
            'sql': f"SELECT COUNT(1) FROM {dest_schema}.{dest_table} WHERE {count_where_clause}",
            'dml': 'Yes',
            'schema': dest_schema,
            'db': dest_service_name,
            'dbtype': 'oracle',
        ...
        return count_statement, update_statement
    else:
        return None, None
