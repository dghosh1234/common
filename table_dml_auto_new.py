...
ENV_BASE_PATH = "/Users/debasishghosh/Desktop/env/"
...
def get_table_metadata(connection, schema, table):
    ...
    # Deduplicate columns by name, preserving order
    seen_cols = set()
    deduped_columns = []
    for col in columns:
    ...
    # Deduplicate key_cols while preserving order
    seen_keys = set()
    deduped_key_cols = []
    for col in key_cols:
...
def main():
    ...
    try:
        ...
        # Support multi-column PK/UK for backup_ids and DDL
        pk_cols = key_cols
        backup_ids = []  # List of tuples for multi-column PK
        backup_id_set = set()
        backup_id_map = {}
        group_rows_map = {}
        used_pk_values = set()  # Set of tuples for multi-column PKs
        for group in router_conditions:
            ...
            cursor = source_connection.cursor()
            # For backup table, collect PKs from target table (multi-col)
            pk_select_sql = f"SELECT {', '.join(pk_cols)} FROM {dest_schema}.{dest_table}{where_str}{order_str}"
            cursor.execute(pk_select_sql)
            ids = [tuple(row) for row in cursor.fetchall()]
            for id_tuple in ids:
                if id_tuple not in backup_id_set:
                    backup_ids.append(id_tuple)
                    backup_id_set.add(id_tuple)
        ...
        # Ensure pk_cols contains each column only once and in table order
        seen_pk_cols = set()
        deduped_pk_cols = []
        for col in pk_cols:
        ...
        # Build WHERE clause for multi-column PKs
        def pk_where_clause(pk_cols, pk_tuple):
            return ' AND '.join(
                f"{col} = '{val}'" if isinstance(val, str) else f"{col} = {val}"
                for col, val in zip(pk_cols, pk_tuple)
            )
        # For backup CTAS, use OR of all PK tuples
        ids_list = ' OR '.join(f"({pk_where_clause(pk_cols, pk_tuple)})" for pk_tuple in backup_ids)
        # Ensure select_cols contains each column only once and in table order, and quote identifiers
        ...
        # Print the generated CTAS SQL for debugging
        print("\n[DEBUG] Generated CTAS SQL for backup table:")
        print(backup_ctas_sql)
        output_filename = f"mock_{dest_table.lower()}.sql"
        ...
        with open(output_filename, 'a', encoding='utf-8') as f:
            ...
            f.write("-- BEGIN\n")
            for pk_tuple in backup_ids:
                where_str = pk_where_clause(pk_cols, pk_tuple)
                delete_sql = f"--   DELETE FROM {dest_schema}.{dest_table} WHERE {where_str};\n"
                f.write(delete_sql)
            f.write("--   COMMIT;\n")
            f.write("-- EXCEPTION\n--   WHEN OTHERS THEN\n--     ROLLBACK;\n--     RAISE;\n-- END;\n")
